...
public class FhomParentNegativeRule extends EtinenTalkingLogicalRule {

    public static final String NAME = "FhomParentNegative";
    private static final String RULE = "0.4: ~Fhom(X,H) & Xinh(X,Z) -> ~Fhom(Z,H)";
    // TODO rephrase 'parent form'?
    private static final String VERBALIZATION = "It is unlikely that a form belongs to a different homologue set than its parent form.";

    // For serialization.
    public FhomParentNegativeRule(String serializedParameters) {
        super(NAME, RULE, VERBALIZATION + ".");
    }

    public FhomParentNegativeRule(PslProblem pslProblem) {
    ...
    public String generateExplanation(EtinenConstantRenderer renderer, String groundingName, String contextAtom,
        ...
        if (contextAtom.equals(fhomCons)) {
            ...
            sb.append(BeliefScale.verbalizeBeliefAsAdjective(rag.getValue(fhomAnte)));
            sb.append(" that \\url[");
            sb.append(escapeForURL(xinhArgs[0]));
            sb.append(" is also a homologue of ").append( renderer == null ? homPeg : renderer.getFormRepresentation(homPeg));
            sb.append("]{").append(fhomAnte).append("}.");
        } else {
            // 'why not lower?'
            sb.append("and it is ").append(BeliefScale.verbalizeBeliefAsAdjective(rag.getValue(fhomCons)));
            sb.append("that \\url[");
